...
if (!function_exists('save_ar_variation')){
    function save_ar_variation( $variation_id, $i ) {
        ...
        if ( isset( $_POST['_usdz_file'.$suffix] ) ) {
            update_post_meta( $variation_id, '_usdz_file'.$suffix, sanitize_text_field( $_POST['_usdz_file'.$suffix] ) );
        }
        ...
        if (( isset( $_POST['_glb_file'.$suffix] ) ) || ( isset( $_POST['_ar_asset_file'.$suffix] ) )):
            if (($_POST['_ar_asset_file'.$suffix] !='' )AND($_POST['_asset_texture_file_0'] !='')){
                ...
                $path_parts = pathinfo(sanitize_text_field( $_POST['_ar_asset_file'.$suffix] ));

                $path_parts['filename'] .= '_' . $_POST['ar_asset_ratio'] . '_' . $_POST['ar_asset_orientation'];
                $path_parts['basename'] = $path_parts['filename'] . '.zip';
            }else{
                $path_parts = pathinfo(sanitize_text_field( $_POST['_glb_file'.$suffix] ));
            }
            /***ZIP***/
            ...
            if (isset($path_parts['extension'])){
                if (strtolower($path_parts['extension'])=='zip'){
                    ...
                    if (  $_POST['_ar_asset_file'.$suffix] !='' ){
                        ...
                        $src_file = download_url( sanitize_text_field( $_POST['_ar_asset_file'.$suffix] ) );
                                ...
                                if (( isset( $_POST['_ar_asset_file'.$suffix] ) )AND( isset( $_POST['_asset_texture_file_0'.$suffix] ) )){
                                    for($i=0;$i<10;$i++){
                                        if (isset($_POST['_asset_texture_file_'.$i])){
                                            $asset_textures[$i]['newfile']=$_POST['_asset_texture_file_'.$i.$suffix];
                                            $asset_textures[$i]['filename']=$_POST['_asset_texture_id_'.$i.$suffix];
                                        }
                                    }
            ...
            if (isset($_POST['_ar_hotspots'.$suffix])){
                if ( $_POST['_ar_hotspots'.$suffix] !='' ){
                    $_ar_hotspots = json_encode($_POST['_ar_hotspots'.$suffix]);
                }
            }
            /***Model Conversion***/
            ...
            if (isset($path_parts['extension'])){
                if ((in_array(strtolower($path_parts['extension']),$allowed_files))AND($zip_gltf=='')){
                    ...
                    $write = fputs( $open,  ar_model_conversion(sanitize_text_field( $_POST['_glb_file'.$suffix] )) ); 
                    fclose( $open );
                    $_POST['_glb_file'.$suffix]= $path_parts['dirname'].'/'.$path_parts['filename'].'.glb';
                }
            }
            
            update_post_meta( $variation_id, '_glb_file'.$suffix, sanitize_text_field( $_POST['_glb_file'.$suffix] ) );
        endif;

        if ((isset( $_POST['_usdz_file'.$suffix] )) OR( isset($_POST['_glb_file'.$suffix]))){
            update_post_meta( $variation_id, '_ar_placement'.$suffix, $_POST['_ar_placement'.$suffix] );
        ...
        foreach ($field_array as $k => $v){
            if ( isset( $_POST[$v.$suffix] ) ) {
                update_post_meta($variation_id, $v.$suffix, $_POST[$v.$suffix] );
            }else{
                update_post_meta($variation_id, $v.$suffix, '');
            }

            //echo $v.$suffix.' = '.$_POST[$v.$suffix].'<br />';
        }

        //wp_die("HERER");
        update_post_meta($variation_id, '_ar_shortcode'.$suffix, '[ardisplay id='.$post_id.']');
